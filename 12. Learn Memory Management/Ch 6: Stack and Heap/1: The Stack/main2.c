...
int main() {
  // call them from the same stack frame
  printMessageOne();
  printMessageTwo();
  printMessageThree();
  return 0;
...
void printStackPointerDiff() {
  ...
  void *current_sp;
  // returns the stack frame address of the current frame which is this functions, going up we increase the level(argument) and it goes up the call stack
